######################################################################################################################################
### R code for: Fossorial adaptations in African mole-rats (Bathyergidae) and the unique appendicular phenotype of naked mole-rats ###
######################################################################################################################################
...
R_OU <- corMartins(1,phy=tree,form=~spp_i,fixed=FALSE) # adjust the tree according to the OU model but let the alpha be estimated rather than fixed
...
m_IFA_bm <- ulam(
   alist(
      ...
      vector[species]:a ~ multi_normal("rep_vector(mu_a,N)",S), 
      matrix[N,N]:S <- sigma_phy_a*R,
      
      # adaptive prior for slopes 
      vector[species]:bBM ~ multi_normal("rep_vector(mu_b,N)",S),
      ...
      sigma ~ exponential(2),
      sigma_phy_a ~ exponential(2),
      sigma_phy_b ~ exponential(2),
...
m_TJI_bm <- ulam(
   alist(
      ...
      # adaptive prior for intercepts 
      vector[N]:a ~ multi_normal("rep_vector(mu_a,N)",S),
      matrix[N,N]:S <- sigma_phy_a*R,
      
      # adaptive prior for slopes 
      vector[N]:bBM ~ multi_normal("rep_vector(mu_b,N)",S),
      matrix[N,N]:S <- sigma_phy_b*R,
